          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!loading && filtered.map((b) => (
                <tr key={b} className="border-t hover:bg-muted/30">
                  <td className="p-2">{b}</td>
                  <td className="p-2">
                    <EditBrandButton oldName={b} onUpdated={load} />
                  </td>
                </tr>
              ))}
            </tbody>
...
function NewBrandButton({ onCreated }: { onCreated: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button>Nueva marca</Button></DialogTrigger>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>Nueva marca</DialogTitle></DialogHeader>
        <div className="grid gap-2">
          <Label>Nombre</Label>
          <Input value={name} onChange={(e)=>setName(e.target.value)} placeholder="Ej. Esymbel" />
...
function EditBrandButton({ oldName, onUpdated }: { oldName: string; onUpdated: () => void }) {
  const [open, setOpen] = React.useState(false);
